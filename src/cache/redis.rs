...
impl RedisCache {
    ...
    pub async fn new(config: &Config, cancel: CancellationToken) -> Result<Self, AppError> {
        ...
        // ignore `config.redis_url`; otherwise use the URL directly.
        let (initial_write_url, sentinel) = if !config.redis_sentinel_nodes.is_empty() {
            let master_password = parse_password_from_url(&config.redis_url);
            let master_addr = resolve_master_via_sentinel(
            ...
            let settings = SentinelSettings {
                nodes: config.redis_sentinel_nodes.clone(),
                master_name: config.redis_sentinel_master_name.clone(),
                master_password,
            ...
            (config.redis_url.clone(), None)
        };

        let write_pool = build_pool(&initial_write_url, config.redis_pool_size)?;
        ...
        let read_url = config
            ...
            .to_string();
        let read_pool = build_pool(&read_url, config.redis_pool_size)?;
        ...
        if cache.sentinel.is_some() {
            cache.spawn_sentinel_monitor(
                config.redis_sentinel_monitor_interval_secs,
                cancel,
            );
        }
    ...
    fn spawn_sentinel_monitor(&self, interval_secs: u64, cancel: CancellationToken) {
        let Some(settings) = self.sentinel.clone() else { return };
